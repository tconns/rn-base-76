...
export const useHandleTabLayout = (index: number, noOfRoutes: number, onLayout: any) => {
  const { routeIndexToTabWidthMap, routeIndexToTabOffsetMap } = useTabLayoutContext()
  ...
  const handleTabLayout = useCallback(
      ...
      function updateTabWidthAndOffset() {
        ...
        if (width !== prevWidth) {
          ...
          for (let i = 0; i <= noOfRoutes; i += 1) {
            const prevRouteIndexWidth = routeIndexToTabWidthMap.value[i - 1] ?? 0
    ...
    [routeIndexToTabWidthMap, index, noOfRoutes, routeIndexToTabOffsetMap, onLayout],
  )
