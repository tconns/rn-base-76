import React, { forwardRef, useCallback, useImperativeHandle, useMemo, useState, MutableRefObject } from 'react'
...
const TabViewCarousel = (props: TabViewCarouselProps, ref: React.Ref<CarouselImperativeHandle>) => {
  const {
    navigationState,
    renderScene,
    layout,
  ...
  const noOfRoutes = useMemo(() => navigationState.routes.length, [navigationState.routes.length])
  ...
  const swipeTranslationX = useSharedValue(-navigationState.index * layout.width)
  const prevRouteTranslationX = useSharedValue(0)

  const [isJumping, setIsJumping] = useState(false)

  const { smallestRouteIndexToRender, largestRouteIndexToRender } = useCarouselRouteIndices(
    currentRouteIndex,
    noOfRoutes,
  )

  const { isLazyLoadingEnabled, handleSceneMount, computeShouldRenderRoute } = useCarouselLazyLoading(
    mode,
    initialRouteIndex,
  ...
  const swipeTranslationAnimatedStyle = useCarouselSwipeTranslationAnimatedStyle(swipeTranslationX)
          ...
          return (
            ...
            >
              ...
              >
                {shouldRender && (
                  <LazyLoader
                    isLazyLoadingEnabled={index !== initialRouteIndex && isLazyLoadingEnabled}
...
export default React.memo(forwardRef(TabViewCarousel))
